...
async function handleGyroEvent(event: GanCubeEvent) {
  if (event.type == "GYRO") {
    ...
    let dot = cubeQuaternion.clone().dot(quat);

    $("#dot").val(dot);

    $("#quaternion").val(
      `x: ${qx.toFixed(3)}, y: ${qy.toFixed(3)}, z: ${qz.toFixed(
        3
      )}, w: ${qw.toFixed(3)}`
    );
    if (event.velocity) {
    ...
    $("#front").val(`Verde aponta para: (${cubeFacesOrientation[0]})`);
    $("#back").val(`Azul aponta para: (${cubeFacesOrientation[1]})`);
    $("#right").val(`Vernelho aponta para: (${cubeFacesOrientation[2]})`);
    $("#left").val(`Laranja aponta para: (${cubeFacesOrientation[3]})`);
    $("#top").val(`Branco aponta para: (${cubeFacesOrientation[4]})`);
    $("#bottom").val(`Amarelo aponta para: (${cubeFacesOrientation[5]})`);
  }
}
